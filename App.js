...
<head>
    ...
    <title>Sistem Informasi Manajemen Sekolah (SIMS)</title>
    ...
    <!-- Babel (Untuk kompilasi JSX di dalam browser) -->
    <script src="https://unpkg.com/@babel/standalone/babel.min.js"></script>
    
    <!-- Phosphor Icons (Pengganti Lucide untuk Standalone HTML) -->
    <script src="https://unpkg.com/@phosphor-icons/web"></script>
</head>
<body class="bg-gray-50 m-0 p-0 font-sans">
    
    <!-- Wadah Aplikasi React -->
    <div id="root"></div>

    <script type="text/babel">
        // Import Hooks dari React Global
        const { useState, useEffect } = React;
        
        // --- ICON MAPPER ---
        // Membuat komponen pembungkus agar ikon tetap bisa dipanggil seperti komponen React
        const createIcon = (baseClass) => ({ size = 24, className = '', color }) => {
            return <i className={`ph ${baseClass} ${className}`} style={{ fontSize: size, color, display: 'inline-block' }}></i>;
        };

        // Memetakan semua icon yang dibutuhkan
        const Users = createIcon('ph-users');
        const Wallet = createIcon('ph-wallet');
        const BookOpen = createIcon('ph-book-open');
        const GraduationCap = createIcon('ph-graduation-cap');
        const Calendar = createIcon('ph-calendar');
        const Settings = createIcon('ph-gear');
        const LogOut = createIcon('ph-sign-out');
        const Menu = createIcon('ph-list');
        const X = createIcon('ph-x');
        const FileText = createIcon('ph-file-text');
        const CheckCircle = createIcon('ph-check-circle');
        const TrendingUp = createIcon('ph-trend-up');
        const LayoutDashboard = createIcon('ph-squares-four');
        const Clock = createIcon('ph-clock');
        const Award = createIcon('ph-medal');
        const BookMarked = createIcon('ph-bookmark-simple');
        const DollarSign = createIcon('ph-currency-dollar');
        const Activity = createIcon('ph-activity');
        const Globe = createIcon('ph-globe');
        const ArrowLeft = createIcon('ph-arrow-left');
        const Trophy = createIcon('ph-trophy');
        const Monitor = createIcon('ph-monitor');
        const Star = createIcon('ph-star');
        const Rss = createIcon('ph-rss');
        ...
        const StatCard = ({ title, value, icon: Icon, color }) => (
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 flex items-center space-x-4 hover:shadow-md transition-shadow">
            <div className={`p-4 rounded-full text-white flex items-center justify-center ${color}`}>
              <Icon size={24} />
            </div>
            <div>
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button className="flex flex-col items-center justify-center p-6 bg-blue-50 rounded-xl text-blue-700 hover:bg-blue-100 transition">
                    ...
                    <span className="font-medium mt-2">Input Nilai</span>
                  </button>
                  <button className="flex flex-col items-center justify-center p-6 bg-green-50 rounded-xl text-green-700 hover:bg-green-100 transition">
                    ...
                    <span className="font-medium mt-2">Data Siswa</span>
                  </button>
                  <button className="flex flex-col items-center justify-center p-6 bg-yellow-50 rounded-xl text-yellow-700 hover:bg-yellow-100 transition">
                    ...
                    <span className="font-medium mt-2">Bank Soal</span>
                  </button>
                  <button className="flex flex-col items-center justify-center p-6 bg-purple-50 rounded-xl text-purple-700 hover:bg-purple-100 transition">
                    ...
                    <span className="font-medium mt-2">Kalender</span>
                  </button>
                </div>
              </div>
            </div>
        ...
        const DashboardWaliSantri = () => (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <select className="bg-white border border-gray-300 text-gray-700 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2 outline-none">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <button className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col items-center justify-center hover:shadow-md transition">
                ...
                <span className="font-medium text-gray-800 mt-2">Jadwal Pelajaran</span>
              </button>
              <button className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col items-center justify-center hover:shadow-md transition">
                <Award size={32} className="text-yellow-500 mb-2" />
                <span className="font-medium text-gray-800 mt-2">Lihat Nilai (KHS)</span>
              </button>
              <button className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col items-center justify-center hover:shadow-md transition">
                <BookMarked size={32} className="text-green-500 mb-2" />
                <span className="font-medium text-gray-800 mt-2">Buku Mutaba'ah</span>
              </button>
              <button className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col items-center justify-center hover:shadow-md transition relative">
                <FileText size={32} className="text-purple-500 mb-2" />
                <span className="font-medium text-gray-800 mt-2">Tugas / PR</span>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
              ...
              <div className="space-y-3">
                 <div className="p-4 bg-red-50 rounded-lg border-l-4 border-red-500 flex justify-between items-center">
                    ...
                    <button className="bg-red-500 text-white px-3 py-1 rounded text-xs font-semibold">Kerjakan</button>
                 </div>
                 <div className="p-4 bg-gray-50 rounded-lg border-l-4 border-gray-300 flex justify-between items-center">
                    ...
                    <button className="bg-white border border-gray-300 text-gray-700 px-3 py-1 rounded text-xs font-semibold hover:bg-gray-100">Lihat</button>
                 </div>
              </div>
            </div>
          </div>
            ...
            return (
              <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                ...
                <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
                  <div className="mx-auto h-16 w-16 text-green-600 flex items-center justify-center">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {roles.map((r) => {
                        ...
                        return (
                          ...
                          >
                            <div className={`p-3 rounded-full text-white flex items-center justify-center ${r.color} mb-3 group-hover:scale-110 transition-transform`}>
                              <Icon size={24} />
                            </div>
                ...
                <div className="p-4 flex flex-col h-full overflow-y-auto">
                  ...
                  <nav className="flex-1 space-y-1">
                    {getMenus().map((menu, index) => (
                      ...
                      >
                        <div className={`mr-3 ${currentView === menu.id ? 'text-green-600' : 'text-gray-400'}`}>
                          <menu.icon size={20} />
                        </div>
                        {menu.label}
                      </a>
